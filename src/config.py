...
def parse_args() -> Config:
    ...
    parser.add_argument(
        "--db-url", type=str, help="PostgreSQL database URL (overrides default)"
    )

    parser.add_argument(
        "--host", type=str, help="Host to bind the server (default: 0.0.0.0)"
    )
    ...
    return Config(db_type=db_type, db_file=db_file, db_url=db_url, host=host, port=port)
